...
/*BeginDocumentation
Name: environment_cliff - non-neuronal environment stimulating and recording from a spiking neuronal network model to perform a cliff walk task
...
#ifndef ENVIRONMENT_CLIFF_H
#define ENVIRONMENT_CLIFF_H
...
namespace nest
{
   ...
   * @ingroup Devices
   */
  class environment_cliff : public Archiving_Node//, public StimulatingDevice<CurrentEvent>
  {
  ...
  public:        
    ...
    environment_cliff();
    environment_cliff(const environment_cliff&);
  ...
  private:
    ...
    struct Parameters_ {
      ...
      /** Vector of x positions and y positions representing the cliff states. */
      std::vector<long_t> cliffx_;
      std::vector<long_t> cliffy_;
      ...
      /** amount of reward in pA */
      double_t special_punishment_;

      /** amount of reward in pA */
      double_t punishment_;
    ...
    /** 
     * Node to which environment_cliff instance belongs.
...
inline  
port environment_cliff::check_connection(Connection& c, port receptor_type)
    {
      DSCurrentEvent e;
      e.set_sender(*this);
      c.check_event(e);
...
inline
  port environment_cliff::connect_sender(SpikeEvent&, port receptor_type)
  {
    if (receptor_type != 0)
...
/*
inline
void environment_cliff::set_status(const DictionaryDatum &d)
{
...
#endif /* #ifndef ENVIRONMENT_CLIFF_H */
