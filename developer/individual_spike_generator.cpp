/*
 *  individual_spike_generator.cpp
 *
 *  EXPERIMENTAL! Just a copy of individual_spike_generator.cpp
 *  with some renames spike_generator -> individual_spike_generator
 *
...
nest::individual_spike_generator::Parameters_::Parameters_()
  : spike_stamps_(),
    spike_offsets_(),
    spike_weights_(),
...
nest::individual_spike_generator::Parameters_::Parameters_(const Parameters_& op)
...
nest::individual_spike_generator::State_::State_()
  : position_(0)
{}
...
void nest::individual_spike_generator::Parameters_::get(DictionaryDatum &d) const
{  
...
void nest::individual_spike_generator::Parameters_::assert_valid_spike_time_and_insert_(double t,
									     const Time& origin,
									     const Time& now)
{
  Time t_spike;
  ...
  else 
  {
	...
	msg << "individual_spike_generator: Time point " << t 
...
void nest::individual_spike_generator::Parameters_::set(const DictionaryDatum& d, State_& s, 
...
nest::individual_spike_generator::individual_spike_generator()
  : Node(),
    device_(), 
    P_(),
    S_()
...
nest::individual_spike_generator::individual_spike_generator(const individual_spike_generator& n)
  : Node(n), 
    device_(n.device_),
    P_(n.P_),
    S_(n.S_)
...
void nest::individual_spike_generator::init_state_(const Node& proto)
{ 
  const individual_spike_generator& pr = downcast<individual_spike_generator>(proto);

  device_.init_state(pr.device_);
  S_ = pr.S_;
}

void nest::individual_spike_generator::init_buffers_()
{ 
  device_.init_buffers();
}

void nest::individual_spike_generator::calibrate()
{
  device_.calibrate();
}
...
void nest::individual_spike_generator::update(Time const & sliceT0, const long_t from, const long_t to)
{
...
void nest::individual_spike_generator::event_hook(DSSpikeEvent& e)
{
