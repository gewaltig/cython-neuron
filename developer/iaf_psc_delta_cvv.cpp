...
/* iaf_psc_delta_cvv is a neuron where the potential jumps on each spike arrival. */

#include "exceptions.h"
#include "iaf_psc_delta_cvv.h"
#include "network.h"
#include "dict.h"
...
nest::iaf_psc_delta_cvv::Parameters_::Parameters_()
  : tau_m_     ( 10.0     ),  // ms
    c_m_       (250.0     ),  // pF
    t_ref_     (  2.0     ),  // ms
    E_L_       (-70.0     ),  // mV
...
nest::iaf_psc_delta_cvv::State_::State_()
  : y0_   (0.0),
    y3_   (0.0),  
    y3_t_spike_plus_(0.0),
    r_    (0),
...
void nest::iaf_psc_delta_cvv::Parameters_::get(DictionaryDatum &d) const
{
...
void nest::iaf_psc_delta_cvv::Parameters_::set(const DictionaryDatum& d)
{
...
void nest::iaf_psc_delta_cvv::State_::get(DictionaryDatum &d, const Parameters_& p) const
{
  def<double>(d, names::V_m, y3_ + p.E_L_); // Membrane potential
}

void nest::iaf_psc_delta_cvv::State_::set(const DictionaryDatum& d, const Parameters_& p)
{
...
nest::iaf_psc_delta_cvv::iaf_psc_delta_cvv()
  : Archiving_Node(), 
    P_(), 
    S_()
{}

nest::iaf_psc_delta_cvv::iaf_psc_delta_cvv(const iaf_psc_delta_cvv& n)
  : Archiving_Node(n), 
    P_(n.P_), 
    S_(n.S_)
{}
...
void nest::iaf_psc_delta_cvv::init_buffers_()
{
...
void nest::iaf_psc_delta_cvv::calibrate()
{
  ...
  //
  // The definition of the refractory period of the iaf_psc_delta_cvv is consistent 
...
void nest::iaf_psc_delta_cvv::update(Time const & origin, 
				 const long_t from, const long_t to)
{
  ...
  for ( long_t lag = from ; lag < to ; ++lag )
  {
	...
	B_.spikes_.get_value(lag);  // clear buffer entry, ignore spike
      ...
      --S_.r_;
      if (S_.r_ == 0) // leaving refractory period 
	S_.y3_ = S_.y3_t_spike_plus_ - (P_.V_th_ - P_.V_reset_); // start value for voltage
    }
...
void nest::iaf_psc_delta_cvv::handle(SpikeEvent & e)
{
...
void nest::iaf_psc_delta_cvv::handle(CurrentEvent& e)
{
  assert(e.get_delay() > 0);
...
void nest::iaf_psc_delta_cvv::handle(PotentialRequest& e)
{
