...
#ifndef POLICY_TH_CONNECTION_H
#define POLICY_TH_CONNECTION_H

/* BeginDocumentation
  Name: policy_th_synapse - Synapse type for dopamine modulated synaptic plasticity. The synapse implements the policy of actor-critic TD learning.   

  Description:
   policy_th_synapse is a connector to create synapses with dopamine modulated plasticity;  the plasticity rule depends on pre- and postsynaptic spikes and on an additional volume signal (dopamine) coming from a non-local neuron pool; the plasticity rule is based on the policy update of actor-critic TD learning.
...
namespace nest
{
  ...
  /**
   * Class containing the common properties for all synapses of type POLICY_TH_Connection.
   */
  class Policy_TH_CommonProperties : public CommonSynapseProperties
    {
      friend class Policy_TH_Connection;
  ...
  /**
   * Class representing an POLICY_TH_ connection with homogeneous parameters, i.e. parameters are the same for all synapses.
   */
  class Policy_TH_Connection : public ConnectionHetWD
  {
   ...
   */
  Policy_TH_Connection();
  
  /**
   * Copy constructor from a property object.
   * Needs to be defined properly in order for GenericConnector to work.
   */
  Policy_TH_Connection(const Policy_TH_Connection &);

  /**
   * Default Destructor.
   */
  virtual ~Policy_TH_Connection() {}
   ...
   * We have to override the base class' implementation, since for POLICY_TH_
   ...
   */
  void send(Event& e, double_t t_lastspike, const Policy_TH_CommonProperties &);
  ...
  void trigger_update_weight(const vector<spikecounter> &dopa_spikes, const Policy_TH_CommonProperties &cp);
  void update_weight(double_t this_update, const Policy_TH_CommonProperties &cp);
  ...
  inline
    void Policy_TH_Connection::trigger_update_weight(const vector<spikecounter> &dopa_spikes, const Policy_TH_CommonProperties &cp)
  {
  ...
  inline
    void Policy_TH_Connection::update_weight(double_t this_update, const Policy_TH_CommonProperties &cp)
    {
      bool plastic = false;
  ...
  if(pre_state_=='a'){
    ...
    if(pre_k_now>=cp.limit1_) pre_state_= 'p';
      ...
      if((plastic) && (post_trace > cp.limita_))
      {
...
inline 
  void Policy_TH_Connection::check_connection(Node & s, Node & r, port receptor_type, double_t t_lastspike)
{
 ...
 */
inline
void Policy_TH_Connection::send(Event& e, double_t t_lastspike, const Policy_TH_CommonProperties &cp)
{
...
#endif // of #ifndef POLICY_TH_CONNECTION_H
