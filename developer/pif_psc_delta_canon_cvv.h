...
#ifndef PIF_PSC_DELTA_CANON_CVV_H
#define PIF_PSC_DELTA_CANON_CVV_H
...
namespace nest{
  ...
  /* BeginDocumentation
     Name: pif_psc_delta_canon_cvv - perfect integrate-and-fire neuron model.

     Description:

     pif_psc_delta_canon_cvv is an implementation of a perfect integrate-and-fire model
     where the potential jumps on each spike arrival. 

     The threshold crossing is followed by an absolute refractory period
     during which the membrane potential is clamped to the resting potential.
     After the refractory period is over, the membrane potential evolution
     continues at V(t_spike-\epsilon) - (V_th - V_reset). This modified
     reset was proposed by Carl van Vreeswijk because this is the limit
     of the neuron, where a finite synaptic time constant is reduced to 0.
     ...
     The pif_psc_delta_canon_cvv neuron does not accept CurrentEvent connections.
     ...
     C_m        double - Differential capacitance of the membrane in pF, determines the effect of constant injected current to cause voltage change dV/dt = I_e/C_m
     ...
     Author:  Oct 2010 Helias; based on work by Plesser, Diesmann, Gewaltig, Morrison, Straube, Eppler
     SeeAlso: iaf_psc_delta
  */

  class pif_psc_delta_canon_cvv:
  public Node
  {
  ...
  public:        
    ...
    */ 
    pif_psc_delta_canon_cvv(const pif_psc_delta_canon_cvv&);
  ...
  private:
    ...
    friend class RecordablesMap<pif_psc_delta_canon_cvv>;
    friend class UniversalDataLogger<pif_psc_delta_canon_cvv>;
    ...
    struct State_ {
      ...
      double_t U_t_spike_plus_;  //!< This is the membrane potential RELATIVE TO RESTING POTENTIAL after the last spike before reset
    ...
    struct Buffers_ {
      ...
      Buffers_(pif_psc_delta_canon_cvv&);
      Buffers_(const Buffers_&, pif_psc_delta_canon_cvv&);
      ...
      UniversalDataLogger<pif_psc_delta_canon_cvv> logger_;
    };
     ...
     */
    struct Variables_ { 
      double_t delta_V_Ie_;     //!< @$ \frac{I_e}{c_m} h @$
   ...
   static RecordablesMap<pif_psc_delta_canon_cvv> recordablesMap_;

  };
  
  inline
    port pif_psc_delta_canon_cvv::check_connection(Connection& c, port receptor_type)
    {
      SpikeEvent e;
      e.set_sender(*this);
      c.check_event(e);
  ...
  inline
    port pif_psc_delta_canon_cvv::connect_sender(SpikeEvent&, port receptor_type)
    {
      if (receptor_type != 0)
  ...
  inline
    port pif_psc_delta_canon_cvv::connect_sender(DataLoggingRequest& dlr, 
					     port receptor_type)
    {
      if (receptor_type != 0)
	throw UnknownReceptorType(receptor_type, get_name());
  ...
  inline 
    Time pif_psc_delta_canon_cvv::get_spiketime() const
    {
      return Time::step(S_.last_spike_step_);
    }

  inline
    void pif_psc_delta_canon_cvv::get_status(DictionaryDatum &d) const
  {
    P_.get(d);
    S_.get(d, P_);
  ...
  inline
    void pif_psc_delta_canon_cvv::set_status(const DictionaryDatum &d)
  {
...
#endif //PIF_PSC_DELTA_CANON_CVV_H
