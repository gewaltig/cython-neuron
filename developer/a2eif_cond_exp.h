...
#ifndef A2EIF_COND_EXP_H
#define A2EIF_COND_EXP_H
...
/* BeginDocumentation
Name: a2eif_cond_exp - Conductance based exponential integrate-and-fire neuron model according to Brette and Gerstner (2005)* with two adaptation terms**.

Description:
a2eif_cond_exp is the adaptive exponential integrate and fire neuron with two adaptation terms for short-term
adaptation as well as long-term adaptation.
...
and
tau_w1 * dw1/dt= a(V-E_L) -w1
tau_w2 * dw2/dt= a(V-E_L) -w2
...
namespace nest
{
   ...
   */
  extern "C"
  int a2eif_cond_exp_dynamics (double, const double*, double*, void*);

  class a2eif_cond_exp:
  ...
  public:        
    ...
    a2eif_cond_exp();
    a2eif_cond_exp(const a2eif_cond_exp&);
    ~a2eif_cond_exp();
  ...
  private:
    ...
    friend int a2eif_cond_exp_dynamics(double, const double*, double*, void*);

    // The next two classes need to be friends to access the State_ class/member
    friend class RecordablesMap<a2eif_cond_exp>;
    friend class UniversalDataLogger<a2eif_cond_exp>;
  ...
  W2       ,  // 4
    ...
    struct Buffers_
    {
      Buffers_(a2eif_cond_exp &);
      Buffers_(const Buffers_ &, a2eif_cond_exp &);

      UniversalDataLogger<a2eif_cond_exp> logger_;    //! Logger for all analog data
    ...
    static RecordablesMap<a2eif_cond_exp> recordablesMap_;

  };


  inline  
  port a2eif_cond_exp::check_connection(Connection &c, port receptor_type)
  {
    SpikeEvent e;
    e.set_sender(*this);
    c.check_event(e);
  ...
  inline
  port a2eif_cond_exp::connect_sender(SpikeEvent &, port receptor_type)
  {
    if (receptor_type != 0)
  ...
  inline
  port a2eif_cond_exp::connect_sender(CurrentEvent &, port receptor_type)
  {
    if (receptor_type != 0)
      throw UnknownReceptorType(receptor_type, get_name());
    return 0;
  ...
  inline
  port a2eif_cond_exp::connect_sender(DataLoggingRequest &dlr, 
				      port receptor_type)
  {
    if (receptor_type != 0)
      throw UnknownReceptorType(receptor_type, get_name());
  ...
  inline
  void a2eif_cond_exp::get_status(DictionaryDatum &d) const
  {
    P_.get(d);
    S_.get(d);
  ...
  inline
  void a2eif_cond_exp::set_status(const DictionaryDatum &d)
  {
