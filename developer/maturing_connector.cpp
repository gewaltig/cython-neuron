...
namespace nest
{
  ...
  MaturingConnector::MaturingConnector(GCMT &cm) :
    GenericConnectorBase< MaturingConnection,
 ...
 */ 
void MaturingConnector::register_connection(Node& s, Node& r)
{
  ...
  if ( !connector_model_.get_common_properties().ignore_multapses_ ||
    ...
    {
      GenericConnectorBase< MaturingConnection, MaturingCommonProperties, GCMT >::register_connection(s, r);
    } 
}
 ...
 */ 
void MaturingConnector::register_connection(Node& s, Node& r, double_t w, double_t d)
{
  // only connect, if we do not ignore multapses
  // or if there is no connection to this target
  if ( !connector_model_.get_common_properties().ignore_multapses_ ||
       connection_exists(r) == invalid_port_ )
    {
      GenericConnectorBase< MaturingConnection, MaturingCommonProperties, GCMT >::register_connection(s, r, w, d);
    } 
}
 ...
 */ 
void MaturingConnector::register_connection(Node& s, Node& r, DictionaryDatum& d)
{
  // only connect, if we do not ignore multapses
  // or if there is no connection to this target
  if ( !connector_model_.get_common_properties().ignore_multapses_ ||
       connection_exists(r) == invalid_port_ )
    {
      GenericConnectorBase< MaturingConnection, MaturingCommonProperties, GCMT >::register_connection(s, r, d);
    } 

}


void MaturingConnector::send(Event& e)
{
  ...
  for (ConnIter it = connections_.begin(); it != connections_.end(); i++)
  {
    ...
    bool synapse_died = it->send(e, t_lastspike_, connector_model_.get_common_properties(), h_, x_);
