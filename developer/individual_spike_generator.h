/*
 *  individual_spike_generator.h
 *
 *  EXPERIMENTAL! copy of spike_generator.h with
 *  bool has_proxies() const {return true;} and renames
 *
...
#ifndef INDIVIDUAL_SPIKE_GENERATOR_H
#define INDIVIDUAL_SPIKE_GENERATOR_H
...
{
  /*BeginDocumentation
    Name: individual_spike_generator - A device which generates spikes from an array with spike-times.

    Synopsis: individual_spike_generator Create -> gid
    ...
    Note that GetStatus will report the spike times that the individual_spike_generator
    ...
    /individual_spike_generator << /spike_times [1.0 1.9999 3.0001] >> Create
    ---> spikes at steps 10 (==1.0ms), 20 (==2.0ms) and 30 (==3.0ms)

    /individual_spike_generator << /spike_times [1.0 1.05 3.0001] >> Create
    ---> error, spike time 1.05 not within tic/2 of step

    /individual_spike_generator << /spike_times [1.0 1.05 3.0001] 
    ...
    /individual_spike_generator << /spike_times [10.0001] >> Create 
    ---> spike time is within tic/2 of step 100, rounded down to 100 thus
         not in the future, spike will not be emitted
    
    /individual_spike_generator << /spike_times [10.0001] /precise_times true >> Create 
    ---> spike at step 101, offset -0.0999 is in the future

    /individual_spike_generator << /spike_times [10.0001 11.0001] /shift_now_spikes true >> Create 
  ...
  */
   ...
   * @ingroup Devices
   */
  class individual_spike_generator : public Node
  {
    
  public:    
    individual_spike_generator();
    individual_spike_generator(const individual_spike_generator&);
...
inline  
port individual_spike_generator::check_connection(Connection& c, port receptor_type)
{
  SpikeEvent e;
  e.set_sender(*this);
  c.check_event(e);
...
inline
void individual_spike_generator::get_status(DictionaryDatum &d) const
{
  P_.get(d);
...
inline
void individual_spike_generator::set_status(const DictionaryDatum &d)
{
...
#endif /* #ifndef INDIVIDUAL_SPIKE_GENERATOR_H */
