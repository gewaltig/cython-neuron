...
/* iaf_psc_delta_canon_cvv is a neuron where the potential jumps on each spike arrival. */
...
namespace nest
{
  ...
  RecordablesMap<iaf_psc_delta_canon_cvv> iaf_psc_delta_canon_cvv::recordablesMap_;
   ...
   */
  template <>
  void RecordablesMap<iaf_psc_delta_canon_cvv>::create()
  {
    // use standard names whereever you can for consistency!
    insert_(names::V_m, &iaf_psc_delta_canon_cvv::get_V_m_);
  }
...
nest::iaf_psc_delta_canon_cvv::Parameters_::Parameters_()
  : tau_m_  ( 10.0    ),  // ms
    c_m_    (250.0    ),  // pF
    t_ref_  (  2.0    ),  // ms
    E_L_    (-70.0    ),  // mV
...
nest::iaf_psc_delta_canon_cvv::State_::State_()
  : U_(0.0), //  or U_ = U_reset_;
    U_t_spike_plus_(0.0), // = U_reset_;
    last_spike_step_(-1),
...
void nest::iaf_psc_delta_canon_cvv::Parameters_::get(DictionaryDatum &d) const
{
...
double nest::iaf_psc_delta_canon_cvv::Parameters_::set(const DictionaryDatum& d)
{
...
void nest::iaf_psc_delta_canon_cvv::State_::get(DictionaryDatum &d, 
...
void nest::iaf_psc_delta_canon_cvv::State_::set(const DictionaryDatum& d, const Parameters_& p, double delta_EL)
{
...
nest::iaf_psc_delta_canon_cvv::Buffers_::Buffers_(iaf_psc_delta_canon_cvv& n)
  : logger_(n)
{}

nest::iaf_psc_delta_canon_cvv::Buffers_::Buffers_(const Buffers_&, iaf_psc_delta_canon_cvv& n)
  : logger_(n)
{}
...
nest::iaf_psc_delta_canon_cvv::iaf_psc_delta_canon_cvv()
  : Node(), 
    P_(), 
    S_(),
    B_(*this)
...
nest::iaf_psc_delta_canon_cvv::iaf_psc_delta_canon_cvv(const iaf_psc_delta_canon_cvv& n)
  : Node(n), 
    P_(n.P_), 
    S_(n.S_),
    B_(n.B_, *this)
...
void nest::iaf_psc_delta_canon_cvv::init_node_(const Node& proto)
{
  const iaf_psc_delta_canon_cvv& pr = downcast<iaf_psc_delta_canon_cvv>(proto);
  P_ = pr.P_;
  S_ = pr.S_;
}

void nest::iaf_psc_delta_canon_cvv::init_state_(const Node& proto)
{
  const iaf_psc_delta_canon_cvv& pr = downcast<iaf_psc_delta_canon_cvv>(proto);
  S_ = pr.S_;
}

void nest::iaf_psc_delta_canon_cvv::init_buffers_()
{
  B_.events_.resize();
  B_.events_.clear(); 
  B_.logger_.reset();
...
void iaf_psc_delta_canon_cvv::calibrate()
{
  B_.logger_.init();
...
void iaf_psc_delta_canon_cvv::update(Time const & origin, 
	...
	*/ 
	...
	if ( S_.is_refractory_ )
	{
	  ...
	  else 
	  {
	    ...
	    // start the membrane potential at V(t_spike+) - (U_th-U_reset).
	    // this relative reset has been proposed by Carl v. Vreeswijk
	    // as the correct limit of tau_syn -> 0
	    S_.U_ = S_.U_t_spike_plus_ - (P_.U_th_ - P_.U_reset_);
	...
	if ( S_.U_ >= P_.U_th_ )
	  {
	    // assert(false); // will never go here, can only spike, if exc input spike
...
void nest::iaf_psc_delta_canon_cvv::propagate_(const double_t dt)
{
...
void nest::iaf_psc_delta_canon_cvv::emit_spike_(Time const &origin, const long_t lag,
					    const double_t offset_U) 
{
  ...
  // if we are in emit_spike, the crossing was due to DC input, to threshold was exactly reached
...
void nest::iaf_psc_delta_canon_cvv::emit_instant_spike_(Time const &origin, const long_t lag,
...
void iaf_psc_delta_canon_cvv::handle(SpikeEvent & e)
{
...
void nest::iaf_psc_delta_canon_cvv::handle(DataLoggingRequest& e)
{
  B_.logger_.handle(e);
}

void iaf_psc_delta_canon_cvv::set_spiketime(Time const & now)
{
