...
#ifndef IAF_PSC_DELTA_NODELAY_H
#define IAF_PSC_DELTA_NODELAY_H
...
namespace nest{
  ...
  */
  ...
  public:        
    ...
    iaf_psc_delta_nodelay();
    iaf_psc_delta_nodelay(const iaf_psc_delta_nodelay&);
  ...
  private:
    ...
    friend class RecordablesMap<iaf_psc_delta_nodelay>;
    friend class UniversalDataLogger<iaf_psc_delta_nodelay>;
     ...
     */
    struct Buffers_ {
      Buffers_(iaf_psc_delta_nodelay &);
      Buffers_(const Buffers_ &, iaf_psc_delta_nodelay &);
      ...
      UniversalDataLogger<iaf_psc_delta_nodelay> logger_;
    };
    ...
    static RecordablesMap<iaf_psc_delta_nodelay> recordablesMap_;
  };
    
  inline
    port iaf_psc_delta_nodelay::check_connection(Connection& c, port receptor_type)
    {
      SpikeEvent e;
      e.set_sender(*this);
      c.check_event(e);
  ...
  inline
    port iaf_psc_delta_nodelay::connect_sender(SpikeEvent&, port receptor_type)
    {
      if (receptor_type != 0)
  ...
  inline
    port iaf_psc_delta_nodelay::connect_sender(CurrentEvent&, port receptor_type)
    {
      if (receptor_type != 0)
	throw UnknownReceptorType(receptor_type, get_name());
      return 0;
  ...
  inline
  port iaf_psc_delta_nodelay::connect_sender(DataLoggingRequest &dlr, 
				     port receptor_type)
  {
    if (receptor_type != 0)
      throw UnknownReceptorType(receptor_type, get_name());
...
inline
void iaf_psc_delta_nodelay::get_status(DictionaryDatum &d) const
{
  P_.get(d);
  S_.get(d, P_);
...
inline
void iaf_psc_delta_nodelay::set_status(const DictionaryDatum &d)
{
...
#endif /* #ifndef IAF_PSC_DELTA_NODELAY_H */
