...
#ifndef IAF_PSC_DELTA_CANON_CVV_H
#define IAF_PSC_DELTA_CANON_CVV_H
...
namespace nest{
  ...
  /* BeginDocumentation
     Name: iaf_psc_delta_canon_cvv - Leaky integrate-and-fire neuron model.

     Description:

     iaf_psc_delta_canon_cvv is an implementation of a leaky integrate-and-fire model
     ...
     The iaf_psc_delta_canon_cvv is the standard model used to check the consistency
     ...
     The iaf_psc_delta_canon_cvv neuron does not accept CurrentEvent connections.
     ...
     optimization levels. A future version of iaf_psc_delta_canon_cvv will probably
  ...
  */
  ...
  public:        
    ...
    */ 
    iaf_psc_delta_canon_cvv(const iaf_psc_delta_canon_cvv&);
  ...
  private:
    ...
    friend class RecordablesMap<iaf_psc_delta_canon_cvv>;
    friend class UniversalDataLogger<iaf_psc_delta_canon_cvv>;
    ...
    struct Buffers_ {
      ...
      Buffers_(iaf_psc_delta_canon_cvv&);
      Buffers_(const Buffers_&, iaf_psc_delta_canon_cvv&);
      ...
      UniversalDataLogger<iaf_psc_delta_canon_cvv> logger_;
    };
   ...
   static RecordablesMap<iaf_psc_delta_canon_cvv> recordablesMap_;

  };
  
  inline
    port iaf_psc_delta_canon_cvv::check_connection(Connection& c, port receptor_type)
    {
      SpikeEvent e;
      e.set_sender(*this);
      c.check_event(e);
  ...
  inline
    port iaf_psc_delta_canon_cvv::connect_sender(SpikeEvent&, port receptor_type)
    {
      if (receptor_type != 0)
  ...
  inline
    port iaf_psc_delta_canon_cvv::connect_sender(DataLoggingRequest& dlr, 
						 port receptor_type)
    {
      if (receptor_type != 0)
	throw UnknownReceptorType(receptor_type, get_name());
  ...
  inline 
    Time iaf_psc_delta_canon_cvv::get_spiketime() const
    {
      return Time::step(S_.last_spike_step_);
    }

  inline
    void iaf_psc_delta_canon_cvv::get_status(DictionaryDatum &d) const
  {
    P_.get(d);
    S_.get(d, P_);
  ...
  inline
    void iaf_psc_delta_canon_cvv::set_status(const DictionaryDatum &d)
  {
...
#endif //IAF_PSC_DELTA_CANON_CVV_H
