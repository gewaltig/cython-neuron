...
/* pif_psc_delta_canon_cvv is a neuron where the potential jumps on each spike arrival. */
...
namespace nest
{
  ...
  RecordablesMap<pif_psc_delta_canon_cvv> pif_psc_delta_canon_cvv::recordablesMap_;
   ...
   */
  template <>
  void RecordablesMap<pif_psc_delta_canon_cvv>::create()
  {
    // use standard names whereever you can for consistency!
    insert_(names::V_m, &pif_psc_delta_canon_cvv::get_V_m_);
  }
...
nest::pif_psc_delta_canon_cvv::Parameters_::Parameters_()
  : c_m_    (250.0),  // 250 pF
    t_ref_  (  2.0    ),  // ms
    E_L_    (-70.0    ),  // mV
    I_e_    (  0.0    ),  // pA
...
nest::pif_psc_delta_canon_cvv::State_::State_()
...
void nest::pif_psc_delta_canon_cvv::Parameters_::get(DictionaryDatum &d) const
{
...
double nest::pif_psc_delta_canon_cvv::Parameters_::set(const DictionaryDatum& d)
{
...
void nest::pif_psc_delta_canon_cvv::State_::get(DictionaryDatum &d, 
...
void nest::pif_psc_delta_canon_cvv::State_::set(const DictionaryDatum& d, const Parameters_& p, double delta_EL)
{
...
nest::pif_psc_delta_canon_cvv::Buffers_::Buffers_(pif_psc_delta_canon_cvv& n)
  : logger_(n)
{}

nest::pif_psc_delta_canon_cvv::Buffers_::Buffers_(const Buffers_&, pif_psc_delta_canon_cvv& n)
  : logger_(n)
{}
...
nest::pif_psc_delta_canon_cvv::pif_psc_delta_canon_cvv()
  : Node(), 
    P_(), 
    S_(),
    B_(*this)
...
nest::pif_psc_delta_canon_cvv::pif_psc_delta_canon_cvv(const pif_psc_delta_canon_cvv& n)
  : Node(n), 
    P_(n.P_), 
    S_(n.S_),
    B_(n.B_, *this)
...
void nest::pif_psc_delta_canon_cvv::init_buffers_()
{
  B_.events_.resize();
  B_.events_.clear(); 
  B_.logger_.reset();
...
void pif_psc_delta_canon_cvv::calibrate()
{
  B_.logger_.init();

  V_.h_ms_ = Time::get_resolution().get_ms(); 

  V_.delta_V_Ie_ = P_.I_e_ / P_.c_m_ * V_.h_ms_; // change of voltage due to consant input current
...
void pif_psc_delta_canon_cvv::update(Time const & origin, 
...
void nest::pif_psc_delta_canon_cvv::propagate_(const double_t dt)
{
  assert(!S_.is_refractory_);  // should not be called if neuron is
			    // refractory

  S_.U_ = P_.I_e_ / P_.c_m_ * dt + S_.U_;
...
void nest::pif_psc_delta_canon_cvv::emit_instant_spike_(Time const &origin, const long_t lag,
...
void nest::pif_psc_delta_canon_cvv::handle(DataLoggingRequest& e)
{
  B_.logger_.handle(e);
}

void pif_psc_delta_canon_cvv::set_spiketime(Time const & now)
{
