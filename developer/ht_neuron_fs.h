#ifndef HT_NEURON_FS_H
#define HT_NEURON_FS_H
...
/* BeginDocumentation
   Name: ht_neuron_fs - Neuron model after Hill & Tononi (2005).
...
namespace nest{
   ...
   */
  extern "C"
  int ht_neuron_fs_dynamics (double, const double*, double*, void*);

  class ht_neuron_fs: public Archiving_Node
  {
  public:
    ht_neuron_fs();
    ht_neuron_fs(const ht_neuron_fs&);
    ~ht_neuron_fs();
  ...
  private:
    ...
    friend int ht_neuron_fs_dynamics(double, const double*, double*, void*);
  ...
  public:
    struct State_ {
      ...
      double_t y_err_[STATE_VEC_SIZE];  //!< error estimate calculated by GSL solver
  ...
  private:
    ...
    friend class RecordablesMap<ht_neuron_fs>;
    friend class UniversalDataLogger<ht_neuron_fs>;
     ...
     */
    struct Buffers_ {
      Buffers_(ht_neuron_fs&);
      Buffers_(const Buffers_&, ht_neuron_fs&);

      UniversalDataLogger<ht_neuron_fs> logger_;
    ...
    static RecordablesMap<ht_neuron_fs> recordablesMap_;
  ...
  inline
    port ht_neuron_fs::check_connection(Connection& c, port receptor_type)
    {
      SpikeEvent e;
      e.set_sender(*this);
      c.check_event(e);
  ...
  inline
    port ht_neuron_fs::connect_sender(SpikeEvent& , port receptor_type)
    {
  ...
  inline
    port ht_neuron_fs::connect_sender(CurrentEvent&, port receptor_type)
    {
      if (receptor_type != 0)
	throw UnknownReceptorType(receptor_type, get_name());
      return 0;
  ...
  inline
    port ht_neuron_fs::connect_sender(DataLoggingRequest& dlr, 
				  port receptor_type)
    {
      if (receptor_type != 0)
	throw UnknownReceptorType(receptor_type, get_name());
