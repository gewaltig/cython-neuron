 ...
 * ---------------------------------------------------------------- */
  ...
  nest::environment_cliff::Parameters_::Parameters_()
    : amp_           (0.0), // pA
      I_inhib_       (0.0), // pA
      nps_           (0.0),
      x_dim_         (0),
 ...
 * ---------------------------------------------------------------- */
 ...
 void nest::environment_cliff::Parameters_::get(DictionaryDatum &d) const
{ 
  ...
  def<double_t> (d, "special_punishment", special_punishment_);
  def<double_t> (d, "punishment", punishment_);
...
void nest::environment_cliff::Parameters_::set(const DictionaryDatum& d)
{
  ...
  updateValue<double_t> (d, "special_punishment", special_punishment_);
  updateValue<double_t> (d, "punishment", punishment_);
  ...
  Name cliffx_name("cliffx");

  if(d->known(cliffx_name))
    {
      const Token& t = d->lookup(cliffx_name);
      ArrayDatum *ad = dynamic_cast<ArrayDatum*>(t.datum());
      if(ad != 0)
	{
	  ad->toVector(cliffx_);
 ...
 Name cliffy_name("cliffy");

  if(d->known(cliffy_name))
    {
      const Token& t = d->lookup(cliffy_name);
      ArrayDatum *ad = dynamic_cast<ArrayDatum*>(t.datum());
      if(ad != 0)
	{
	  ad->toVector(cliffy_);
...
/* ---------------------------------------------------------------- 
 * Default and copy constructor for environment_cliff
 * ---------------------------------------------------------------- */

  nest::environment_cliff::environment_cliff()
    : Archiving_Node(),// StimulatingDevice<CurrentEvent>(), 
      P_(),
      node_(*this)
  {}

  nest::environment_cliff::environment_cliff(const environment_cliff& n)
    : Archiving_Node(n), 
      P_(n.P_),
      node_(n)    
  {}
...
void nest::environment_cliff::init_buffers_()
{
...
void nest::environment_cliff::calibrate()
{
...
void nest::environment_cliff::finalize()
 {
   if ( B_.fs_.is_open() )
   {
...
void nest::environment_cliff::set_status(const DictionaryDatum &d)
{
...
const std::string nest::environment_cliff::build_filename_() const
{
...
void nest::environment_cliff::update(Time const &, const long_t from, const long_t to)
{
...
void nest::environment_cliff::event_hook(DSCurrentEvent& e)
{
...
void nest::environment_cliff::handle(SpikeEvent & e)
{
...
void nest::environment_cliff::check_ref_period(){
  if(B_.ref_period_>0){
    B_.ref_period_-=B_.h; 
    B_.spiked_=true;
  }
...
void nest::environment_cliff::move(int_t direction){
  ...
  if(P_.y_dim_>1){
    ...
    case 3 : go_south(); break;
      //default : std::cout<< "Environment_Cliff->Move-Error: Ungueltige Bewegung or two spikes at same time!! direction:"<< direction<<std::endl;
    }
  }
  ...
  //set_reward();
  //if state is neither reward nor cliff state
  B_.reward_=P_.punishment_;

  if((P_.special_reward_!=0) || (P_.special_punishment_!=0)) special_moves();
  B_.prev_state_=B_.state_;
  B_.state_=pos(B_.agentx_,B_.agenty_,P_.x_dim_);
  
}

void nest::environment_cliff::go_south(){
  B_.agenty_++;
...
void nest::environment_cliff::go_north(){
  B_.agenty_--;
  if(pos_valid()) B_.wall_=false;
  else {
    B_.agenty_++;
...
void nest::environment_cliff::go_west(){
  B_.agentx_--;
  if(pos_valid()) B_.wall_=false;
  else {
    B_.agentx_++;
...
void nest::environment_cliff::go_east(){
  B_.agentx_++;
  if(pos_valid()) B_.wall_=false;
  else{
    B_.agentx_--;
...
bool nest::environment_cliff::pos_valid(){
...
int nest::environment_cliff::pos(int_t x, int_t y, int_t columns){
  return y*columns+x;
}

void nest::environment_cliff::special_moves(){
  ...
  //if state is reward state, set reward to special reward
...
void nest::environment_cliff::random_position(){
  double_t xpos, ypos;
...
void nest::environment_cliff::put_agent(int x, int y){
  B_.agentx_=x;
...
void nest::environment_cliff::plot_agent(){
  ...
  /*
void nest::environment_cliff::open_file_()
{
  std::ostringstream basename;
  basename << "steps";
