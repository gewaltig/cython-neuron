...
namespace nest{
  ...
  RecordablesMap<ht_neuron_fs> ht_neuron_fs::recordablesMap_;

  template <>
  void RecordablesMap<ht_neuron_fs>::create()
  {
    insert_(names::V_m, &ht_neuron_fs::get_y_elem_<ht_neuron_fs::State_::VM>);
    insert_(Name("Theta"), &ht_neuron_fs::get_y_elem_<ht_neuron_fs::State_::THETA>);
    insert_(Name("g_AMPA"), &ht_neuron_fs::get_y_elem_<ht_neuron_fs::State_::G_AMPA>);
    insert_(Name("g_NMDA"), &ht_neuron_fs::get_y_elem_<ht_neuron_fs::State_::G_NMDA>);
    insert_(Name("g_GABAA"), &ht_neuron_fs::get_y_elem_<ht_neuron_fs::State_::G_GABA_A>);
    insert_(Name("g_GABAB"), &ht_neuron_fs::get_y_elem_<ht_neuron_fs::State_::G_GABA_B>); 
    insert_(Name("r_potassium"), &ht_neuron_fs::get_r_potassium_);
    insert_(Name("g_spike"), &ht_neuron_fs::get_g_spike_);
    insert_(Name("I_NaP"), &ht_neuron_fs::get_I_NaP_); 
    insert_(Name("I_KNa"), &ht_neuron_fs::get_I_KNa_); 
    insert_(Name("I_T"), &ht_neuron_fs::get_I_T_); 
    insert_(Name("I_h"), &ht_neuron_fs::get_I_h_); 
 }
 ...
 * ---------------------------------------------------------------- */
  ...
  extern "C"
  inline int ht_neuron_fs_dynamics (double, const double y[], double f[], 
				void* pnode)
  { 
    // shorthand
    typedef nest::ht_neuron_fs::State_ S;

    // get access to node so we can almost work as in a member class
    assert(pnode);
    nest::ht_neuron_fs& node =  *(reinterpret_cast<nest::ht_neuron_fs*>(pnode));
  ...
  nest::ht_neuron_fs::Parameters_::Parameters_()
    : E_Na           ( 30.0 ), // 30 mV
      E_K            (-90.0 ), // -90 mV
      g_NaL          (  0.2 ), // 0.2
      g_KL           (  1.0 ), // 1.0 - 1.85
  ...
  nest::ht_neuron_fs::State_::State_()
    : r_potassium_(0),
      g_spike_(false),
      I_NaP_(0.0),
      I_KNa_(0.0),
  ...
  nest::ht_neuron_fs::State_::State_(const Parameters_& p)
    : r_potassium_ (0),
      g_spike_     (false),
      I_NaP_(0.0),
      I_KNa_(0.0),
  ...
  nest::ht_neuron_fs::State_::State_(const State_& s)
  ...
  nest::ht_neuron_fs::State_& nest::ht_neuron_fs::State_::operator=(const State_& s)
  {
    if(this == &s)
      {
	return *this;
  ...
  void nest::ht_neuron_fs::Parameters_::get(DictionaryDatum &d) const
  {
  ...
  void nest::ht_neuron_fs::Parameters_::set(const DictionaryDatum& d)
  {
  ...
  void nest::ht_neuron_fs::State_::get(DictionaryDatum &d) const
  {
    def<double_t>(d, "V_m",     y_[VM]); // Membrane potential
    def<double_t>(d, "Theta",   y_[THETA]); // Threshold
  }

  void nest::ht_neuron_fs::State_::set(const DictionaryDatum& d, 
				   const Parameters_&)  
  {
  ...
  nest::ht_neuron_fs::Buffers_::Buffers_(ht_neuron_fs& n)
    : logger_(n),
  ...
  nest::ht_neuron_fs::Buffers_::Buffers_(const Buffers_&, ht_neuron_fs& n)
    : logger_(n),
      spike_inputs_(std::vector<RingBuffer>(SUP_SPIKE_RECEPTOR-1)),
      s_(0)
  {
  ...
  nest::ht_neuron_fs::ht_neuron_fs(const ht_neuron_fs& n)
    : Archiving_Node(n), 
      P_(n.P_), 
      S_(n.S_),
      B_(n.B_,*this)
  ...
  void nest::ht_neuron_fs::init_buffers_()
  {
    // Reset spike buffers.
    ...
    B_.sys_.function  = ht_neuron_fs_dynamics; 
    B_.sys_.jacobian  = 0;
  ...
  nest::double_t nest::ht_neuron_fs::get_synapse_constant(nest::double_t Tau_1, 
						      nest::double_t Tau_2,
						      nest::double_t g_peak)
  {
    // Factor used to account for the missing 1/((1/Tau_2)-(1/Tau_1)) term
    // in the ht_neuron_fs_dynamics integration of the synapse terms. 
  ...
  void nest::ht_neuron_fs::calibrate()
  {
  ...
  void nest::ht_neuron_fs::get_status(DictionaryDatum &d) const
  {
    P_.get(d);
    S_.get(d);
  ...
  void nest::ht_neuron_fs::set_status(const DictionaryDatum &d)
  {
  ...
  void ht_neuron_fs::update(Time const & origin, const long_t from, 
			const long_t to)
  {
	...
	const int status = gsl_odeiv_step_apply(B_.s_, 
				  0.0,                  // from t...
				  B_.step_,             // ...to t=t+h
				  S_.y_,	        // neuron state
				  S_.y_err_,            // error estimate
				  0, 0,                 // optional dydt
  ...
  void nest::ht_neuron_fs::handle(SpikeEvent & e)
  {
  ...
  void nest::ht_neuron_fs::handle(CurrentEvent& e)
  {
    assert(e.get_delay() > 0);
  ...
  void nest::ht_neuron_fs::handle(DataLoggingRequest& e)
  {
    B_.logger_.handle(e);
  }
}
