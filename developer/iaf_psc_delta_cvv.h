...
#ifndef IAF_PSC_DELTA_CVV_H
#define IAF_PSC_DELTA_CVV_H
...
namespace nest{
  ...
  /* BeginDocumentation
     Name: iaf_psc_delta_cvv - Leaky integrate-and-fire neuron model.
  ...
  */
  ...
  public:        
    ...
    iaf_psc_delta_cvv();
    iaf_psc_delta_cvv(const iaf_psc_delta_cvv&);
  ...
  private:
    ...
    struct State_ {
      ...
      double_t     y3_t_spike_plus_; //!< This is the membrane potential RELATIVE TO RESTING POTENTIAL after last spike before reset
  ...
  inline
    port iaf_psc_delta_cvv::check_connection(Connection& c, port receptor_type)
    {
      SpikeEvent e;
      e.set_sender(*this);
      c.check_event(e);
  ...
  inline
    port iaf_psc_delta_cvv::connect_sender(SpikeEvent&, port receptor_type)
    {
      if (receptor_type != 0)
  ...
  inline
    port iaf_psc_delta_cvv::connect_sender(CurrentEvent&, port receptor_type)
    {
      if (receptor_type != 0)
	throw UnknownReceptorType(receptor_type, get_name());
      return 0;
  ...
  inline
    port iaf_psc_delta_cvv::connect_sender(PotentialRequest& pr, port receptor_type)
    {
      if (receptor_type != 0)
	throw UnknownReceptorType(receptor_type, get_name());
...
inline
void iaf_psc_delta_cvv::get_status(DictionaryDatum &d) const
{
  P_.get(d);
  S_.get(d, P_);
...
inline
void iaf_psc_delta_cvv::set_status(const DictionaryDatum &d)
{
...
#endif /* #ifndef IAF_PSC_DELTA_CVV_H */
