...
/* iaf_psc_delta_nodelay is a neuron where the potential jumps on each spike arrival. */
...
namespace nest
{
  ...
  RecordablesMap<iaf_psc_delta_nodelay> iaf_psc_delta_nodelay::recordablesMap_;

  // Override the create() method with one call to RecordablesMap::insert_() 
  // for each quantity to be recorded.
  template <>
  void RecordablesMap<iaf_psc_delta_nodelay>::create()
  {
    // use standard names whereever you can for consistency!
    insert_(names::V_m, &iaf_psc_delta_nodelay::get_V_m_);
  }
...
nest::iaf_psc_delta_nodelay::Parameters_::Parameters_()
  : tau_m_     ( 10.0     ),  // ms
    c_m_       (250.0     ),  // pF
    t_ref_     (  2.0     ),  // ms
    E_L_       (-70.0     ),  // mV
...
nest::iaf_psc_delta_nodelay::State_::State_()
  : y0_   (0.0),
    y3_   (0.0),  
    r_    (0),
...
void nest::iaf_psc_delta_nodelay::Parameters_::get(DictionaryDatum &d) const
{
...
double nest::iaf_psc_delta_nodelay::Parameters_::set(const DictionaryDatum& d)
{
...
void nest::iaf_psc_delta_nodelay::State_::get(DictionaryDatum &d, const Parameters_& p) const
{
  def<double>(d, names::V_m, y3_ + p.E_L_); // Membrane potential
}

void nest::iaf_psc_delta_nodelay::State_::set(const DictionaryDatum& d, const Parameters_& p, double delta_EL)
{
...
nest::iaf_psc_delta_nodelay::Buffers_::Buffers_(iaf_psc_delta_nodelay &n)
  : logger_(n)
{}

nest::iaf_psc_delta_nodelay::Buffers_::Buffers_(const Buffers_ &, iaf_psc_delta_nodelay &n)
  : logger_(n)
{}
...
nest::iaf_psc_delta_nodelay::iaf_psc_delta_nodelay()
  : Archiving_Node(), 
    P_(), 
    S_(),
    B_(*this)
...
nest::iaf_psc_delta_nodelay::iaf_psc_delta_nodelay(const iaf_psc_delta_nodelay& n)
  : Archiving_Node(n), 
    P_(n.P_), 
    S_(n.S_),
    B_(n.B_, *this)
...
void nest::iaf_psc_delta_nodelay::init_buffers_()
{
...
void nest::iaf_psc_delta_nodelay::calibrate()
{
  B_.logger_.init();
  ...
  //
  // The definition of the refractory period of the iaf_psc_delta_nodelay is consistent 
...
void nest::iaf_psc_delta_nodelay::update(Time const & origin, 
				 const long_t from, const long_t to)
{
  ...
  for ( long_t lag = from ; lag < to ; ++lag )
  {
    if ( S_.r_ == 0 )
    {
      ...
      S_.y3_ += B_.spikes_.get_value(lag);
...
void nest::iaf_psc_delta_nodelay::handle(SpikeEvent & e)
{
...
void nest::iaf_psc_delta_nodelay::handle(CurrentEvent& e)
{
  assert(e.get_delay() > 0);
...
void nest::iaf_psc_delta_nodelay::handle(DataLoggingRequest &e)
{
  B_.logger_.handle(e);
}
