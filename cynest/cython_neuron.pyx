...
cdef SpecialFunctionsS spFctS = SpecialFunctionsS()
...
class Neuron:
    ...
    def get_ms_on_resolution(self):
        return spFctS.get_msFct(0, -1, -1)

    def get_ms_on_tics(self, tics):
        return spFctS.get_msFct(1, tics, -1)

    def get_ms_on_steps(self, steps):
        return spFctS.get_msFct(2, steps, -1)

    def get_tics_on_resolution(self):
        return spFctS.get_tics_or_stepsFct(0, 1, -1, -1)

    def get_tics_on_steps(self, steps):
        return spFctS.get_tics_or_stepsFct(2, 1, steps, -1)

    def get_tics_on_ms(self, ms):
        return spFctS.get_tics_or_stepsFct(3, 1, -1, ms)

    def get_tics_on_ms_stamp(self, ms_stamp):
        return spFctS.get_tics_or_stepsFct(4, 1, -1, ms_stamp)

    def get_steps_on_resolution(self):
        return spFctS.get_tics_or_stepsFct(0, 2, -1, -1)

    def get_steps_on_tics(self, tics):
        return spFctS.get_tics_or_stepsFct(1, 2, tics, -1)

    def get_steps_on_ms(self, ms):
        return spFctS.get_tics_or_stepsFct(3, 2, -1, ms)

    def get_steps_on_ms_stamp(self, ms_stamp):
        return spFctS.get_tics_or_stepsFct(4, 2, -1, ms_stamp)

    def get_modulo(self, value):
        return spFctS.get_scheduler_valueFct(0, value)

    def get_slice_modulo(self, value):
        return spFctS.get_scheduler_valueFct(1, value)

    def get_min_delay(self):
        return spFctS.get_scheduler_valueFct(2, -1)

    def get_max_delay(self):
        return spFctS.get_scheduler_valueFct(3, -1)
...
cdef public void putSpecialFunctionsS(double (*get_ms)(int, long, double), long (*get_tics_or_steps)(int, int, long, double), unsigned int (*get_scheduler_value)(int, unsigned int)) with gil:
    spFctS.get_msFct = get_ms
    spFctS.get_tics_or_stepsFct = get_tics_or_steps
    spFctS.get_scheduler_valueFct = get_scheduler_value
