...
void nest::Scheduler::simulate(Time const & t)
{
  ...
  {
    t_start = Communicator::get_wtime();
  }
  ...
  if ( Communicator::get_rank() == 0 )
  {
    double_t t_end = Communicator::get_wtime();
    std::cout << "SIMULATE\t" << t_end-t_start << std::endl;
  }
}
...
void nest::Scheduler::serial_update()
{
  ...
  do
  {
    ...
    if ( static_cast<ulong_t>(to_step_) == min_delay_ ) // gather only at end of slice
    {
      if ( Communicator::get_rank() == 0 )
      {
        double_t t_start = Communicator::get_wtime();
        gather_events_();
        double_t t_end = Communicator::get_wtime();
        std::cout << "GATHER\t" << t_end-t_start << std::endl;
      }
      else
        gather_events_();
    }
